...
public class WakeUpUtil {
	 ...
	 */
	private void wakeUp(String[] hwAddresses) throws IOException {
		...
		for (int i = 0; i < hwAddresses.length; i++) {
			...
			log.info(MessageFormat.format("Sending 'magic packet' to {0}", new Object[]{hwAddresses[i]}));
		...
		log.info(MessageFormat.format("Sucessfully send {0} 'magic packet(s)'", new Object[]{new Integer(hwAddresses.length)}));
	...
	public static void main(String[] args){
		try {
			WakeUpUtil util = new WakeUpUtil();
			util.wakeUp("00:13:21:c9:88:25");
