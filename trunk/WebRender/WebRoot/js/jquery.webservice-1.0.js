...
// Name: WSVisit
jQuery.WSVisit = function(options){
	try{
		var settings = {url: "/WebRender/services/"+options.type+"?wsdl",
						dataType: "xml",
						nameSpace: "http://www.animationsp.com/"
					   };
		$.extend(settings,options);
		...
		$.webservice({
			...
			success:function(data,textStatus){
				 $(data).find(settings.methodName+"Return").each(function(){
				 	 var result = $(this).text();
				 	
				 	 if (result=="NoRight" || result.search("Failure")!=-1 ){
				 	 	alert(result);
				 	 }
				 	 else{
